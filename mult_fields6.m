...
function [r1,r2,r3,r4,r5,r6] = mult_fields6( sc, st_dense, x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25,x26,x27,x28,x29,x30,x31,x32,x33,x34,x35,x36,x37,x38,x39);

i_nf = 1;
num_fields = eval(['x' num2str(i_nf) '(1)']);
i_nw = i_nf+1+4*num_fields;
i_mm = i_nw+1+eval(['x' num2str(i_nw)]);
meas = eval(['x' num2str(i_mm)]);
num_meas = size(eval(['x' num2str(i_mm)]),1);
i_re = i_mm+1+2*num_meas;
eval(['opts = x' num2str(nargin-2) ';']);
if (size(opts,1)<=1),
  opts = [opts 1 1 1 1 1];
end;

if (num_fields ~= size(eval(['x' num2str(i_mm)]),2)),
  error( 'Inconsistent # fields and measurement matrix columns.' );
end;
if (num_meas<1),
  error( 'Must have at least one set of measurements.' );
end;
if (eval(['x' num2str(i_nw)])<max(max(-meas))),
...
for m=1:num_meas,
  if sum(abs(meas_siz-eval(['size(x' num2str(i_mm+m) ')'])))>0.5,
    error( 'All measurement planes must be of the same size.' );
  end;
  if sum(abs(meas_siz-eval(['size(x' num2str(i_mm+num_meas+m) ')'])))>0.5,
    error( 'Measurement error variance planes must have the same size as measurements.' );
  end;

  eval(['x' num2str(i_mm+m) ' = dense_to_overlap( x' num2str(i_mm+m) ', ov, 2, 2 );']);
  eval(['x' num2str(i_mm+num_meas+m) ' = dense_to_overlap( x' num2str(i_mm+num_meas+m) ' .* sum_to_dense( dense_to_overlap( ones(size(x' num2str(i_mm+num_meas+m) ')), ov, 2, 2 ), ov, 2, 2 ), ov, 2, 2 );']);

  eval(['x' num2str(i_mm+m) ' = x' num2str(i_mm+m) '(:)'';']);
  eval(['x' num2str(i_mm+num_meas+m) ' = x' num2str(i_mm+num_meas+m) '(:)'';']);
end;
for m=1:(eval(['x' num2str(i_nw)])),
  eval(['x' num2str(i_nw+m) ' = dense_to_overlap( x' num2str(i_nw+m) ', ov, 2, 2 );']);
end;
...
for m=1:num_meas,
  a = find(eval(['x' num2str(i_mm+num_meas+m)])>0);
  yd(a) = yd(a)+1;
end;
...
for m=1:num_meas,
  ...
  for n=1:num_fields,
    if (meas(m,n) >= 0),
      ...
      c(1+(n-1)*yd(a)+ydt(a)+(a-1)*num_fields*num_meas) = eval(['x' num2str(i_nw-round(meas(m,n))) '(a)']);
    end;
  end;
  ydt(a) = ydt(a)+1;
  obs((a-1)*num_meas+ydt(a)) = eval(['x' num2str(i_mm+m) '(a)']);
  cov((a-1)*num_meas+ydt(a)) = eval(['x' num2str(i_mm+num_meas+m) '(a)']);
end;
...
if (opts(4)>0),
  % Interface 6
comstr = [comstr ', x' num2str(i_nf) ', '];
for i=1:num_fields,
  eval(['stcl = x' num2str(i_nf+i*4-2) ';']);
  if (size(stcl,1)==1),
    if length(stcl)<1.5,
  ...
  % Interface 7
if (opts(5)<0),
comstr = [comstr ', x' num2str(i_nf) ', -opts(4), -opts(5), '];
else
comstr = [comstr ', x' num2str(i_nf) ', -opts(4), '];
end;
comstr = [comstr '['];
for i=1:num_fields,
  comstr = [comstr 'x' num2str(i_nf+i*4-1) ' '];
end;
comstr = [comstr '],'];
if (eval(['size(x' num2str(i_nf+1*4-2) ',1)'])==5),
% user has supplied details for interface 7
  comstr = [comstr 'x' num2str(i_nf+i*4-2) '(1,:), x' num2str(i_nf+i*4-2) '(2,:), x' num2str(i_nf+i*4-2) '(3:5,:),'];
else,
% take as standard parameters for interface 6 and translate
comstr = [comstr '['];
for i=1:num_fields,
  comstr = [comstr 'st_dense * x' num2str(i_nf+i*4-1) '/ x' num2str(i_nf+i*4-2) '(1) '];
end;
comstr = [comstr '],['];
for i=1:num_fields,
  if (eval(['size(x' num2str(i_nf+i*4-2) ',1)'])) > 2.5,
    comstr = [comstr 'x' num2str(i_nf+i*4-2) '(1) / x' num2str(i_nf+i*4-2) '(3) '];
  else
    comstr = [comstr '1 '];
  end;
end;
...
comstr = [comstr '['];
for i=1:num_fields,
  comstr = [comstr 'x' num2str(i_nf+i*4-3) ' '];
end;
comstr = [comstr '], x' num2str(i_re) ', '];

if eval(['(size(x' num2str(i_re) ') == size(x' num2str(i_re+1) '))']),
  comstr = [comstr 'x' num2str(i_re+1) ', x' num2str(i_re+2) ', num_fields '];
else,
  comstr = [comstr 'x' num2str(i_re+1) ', num_fields '];
end;
for i=1:num_fields,
  comstr = [comstr ', x' num2str(i_nf+i*4)];
end;
for i=1:num_fields,
  comstr = [comstr ', x' num2str(i_nf+i*4-1)];
end;
