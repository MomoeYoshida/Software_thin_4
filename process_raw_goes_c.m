function [ok]= process_raw_goes_c(stream, year, day, ref_sst, sst_variability, sst_check);

% Program to process GOES E/W data
...
start_day_night=par_info.start_day_night; %0, daytime
end_day_night=par_info.end_day_night; %1, nighttime
...
% Set up geographical grids at 0.05 deg spatial resolution to store SSTs
...
% dumparray=zeros(spatial_resolution); what's this for?
...
goes_type = 'e'
goes_name = ['goes' goes_type];

% Read a bias file for corresponding time (day and night).
for filetype=start_day_night:end_day_night    %  Day and/or night cases...
   
   switch filetype
      case 0
         messagelabel='Daytime'; % why value assigned to unused variable??
         cmask=0;
         datalabel_day = [goes_name '_day_'];
...
% Load up appropriate biases from yesterday's biases file.
      ...
      case 1
         ...
         cmask=0;
         datalabel_night= [goes_name '_night_'];
...
message2(['*** Processing ' goes_name])


% Close logfile so that mex-file can open it

message2(['in process_raw_goes_c.m about to call process_raw_geo_mex'])
close_logfile;


% Call the C-MEX file "process_raw_geo_mex.mexmaca64".
[run_ok day_there sst_day stdvals_day gridcount_day pixel_extent_day ...
 night_there sst_night stdvals_night gridcount_night pixel_extent_night] ...
						    = process_raw_geo_mex(goes_type, year, day,...
                                                      ref_sst,...
                                                      sst_variability,...
                                                      sst_check, bias_day,...
...
if(sst_check) %1, default, quality controled
   check_label='';
else
   check_label='raw_';
end
...
pixel_extent2='(901:3000,1:3000)'	%  GOES SST pixel extent
